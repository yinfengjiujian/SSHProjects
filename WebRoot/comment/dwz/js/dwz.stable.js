...
(function($){
	$.fn.jTable = function(options){
		return this.each(function(){
			//add  by duanml 2016-12-28  增加table中行的双击、单击事件*******start******//
			var onclickrow = options.onclickrow;
			var ondblclickrow = options.ondblclickrow;
			var time = null;   //控制鼠标单击和双击时，处理进入单击事件或双击事件
			//add  by duanml 2016-12-28  增加table中行的双击、单击事件*******end******//
			...
			$trs.hoverClass().each(function(){
				...
				$tr.click(function(){
                    ...
                    time = setTimeout(function(){
                    	var cllesTwoValue = $tr.find("td:first").next().text();
						if(onclickrow != null && onclickrow != "undefined" && onclickrow.indexOf("(") != -1){
							var endLength = onclickrow.indexOf("(");
							var func = onclickrow.substr(0,endLength);
							var newfunc = func + "('" + cllesTwoValue + "');";
                      		eval(newfunc);
	                    }
                    },300);
					//add  by duanml 2016-12-28  增加table中行的双击、单击事件*******end******//
				...
				//add  by duanml 2016-12-28  增加table中行的双击事件*******start******//
				$tr.dblclick(function(){
					clearTimeout(time);
					var cllesTwoValue = $tr.find("td:first").next().text();
					if(ondblclickrow != null && ondblclickrow != "undefined" && ondblclickrow.indexOf("(") != -1){
						var endLength = ondblclickrow.indexOf("(");
					  	var func = ondblclickrow.substr(0,endLength);
                      	var newfunc = func + "('" + cllesTwoValue + "');";
                      	eval(newfunc);
                    }
				});
				//add  by duanml 2016-12-28  增加table中行的双击事件*******end******//
			});
