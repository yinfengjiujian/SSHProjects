...
function initUI($p){
	//tables
	if ($.fn.jTable){
		//modify  by duanml 2016-12-28  增加table中行的双击、单击事件*******start******//
		//$("table.table", $p).jTable();
		$("table.table", $p).each(function(){
	      var $this=$(this);
	      var opts={};
	      if ($this.attr("onclickrow")) opts.onclickrow= $this.attr("onclickrow");
	      if ($this.attr("ondblclickrow")) opts.ondblclickrow= $this.attr("ondblclickrow");
	      $this.jTable(opts);
	    });
	    //modify  by duanml 2016-12-28  增加table中行的双击、单击事件*******end******//
